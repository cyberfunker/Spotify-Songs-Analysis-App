...
raw_data = busca_artista(search) # SEARCH ARTIST DATA ONTHE  SPOTIFY API (see app_spotipy.py)
...
dados = raw_data[["album", "name", "Acousticness", "Danceability", "Energy", "Instrumentalness", "Liveness", "Loudness", "Speechiness","Valence", "Tempo", "Popularity"]]

atributos = ["Acousticness", "Danceability", "Energy", "Instrumentalness", "Liveness", "Loudness", "Speechiness","Valence", "Tempo", "Popularity"]
...
filtro_música = dados["name"].sort_values().unique()
...
if album_check and not musica_check:
  ...
  if album_filter != [] and len(album_filter) <= 10:
    ...
    for i in album_filter:
      ...
      fig1.add_trace(go.Scatterpolar(r=df_album.loc[i, atributos_01], theta=atributos_01, fill="toself",name=i,mode="markers", marker_size=4, marker_color=px.colors.sequential.Viridis[album_filter.index(i)]))
    ...
    fig2 = px.bar(df_name_album, x=df_name_list, y=atributo, color="Popularity",color_continuous_scale=px.colors.sequential.Sunsetdark,labels={'x':''})
    ...
    fig3 = px.scatter(df_name_album, x=atributo_x, y=atributo_y, color="Popularity",color_continuous_scale=px.colors.sequential.Sunsetdark)
    st.write(fig3)
...
if musica_check and not album_check:
  song_filter = st.sidebar.multiselect('Choose Tracks: ', filtro_música)
...
if (not album_check and not musica_check) or (album_check and musica_check):
  ...
  fig8 = px.bar(df_album, x=filtro_album, y=atributo, color="Popularity",color_continuous_scale=px.colors.sequential.Sunsetdark,labels= {'x':''})
  st.write(fig8)
  ...
  fig9 = px.scatter(df_album, x=atributo_x, y=atributo_y, color="Popularity",color_continuous_scale=px.colors.sequential.Sunsetdark, hover_data=[df_album.index])
  st.write(fig9)

  st.markdown("#### By track")
  fig10 = px.scatter(df_name, x=atributo_x, y=atributo_y, color="Popularity",color_continuous_scale=px.colors.sequential.Sunsetdark, hover_data=[df_name.index])
  st.write(fig10)
...
st.sidebar.write("A confidence measure from 0.0 to 1.0 of whether the track is acoustic. 1.0 represents high confidence the track is acoustic.")

st.sidebar.subheader("Danceability")
st.sidebar.write("Danceability describes how suitable a track is for dancing based on a combination of musical elements including tempo, rhythm stability, beat strength, and overall regularity. A value of 0.0 is least danceable and 1.0 is most danceable.")

st.sidebar.subheader("Energy")
st.sidebar.write("Energy is a measure from 0.0 to 1.0 and represents a perceptual measure of intensity and activity. Typically, energetic tracks feel fast, loud, and noisy. For example, death metal has high energy, while a Bach prelude scores low on the scale. Perceptual features contributing to this attribute include dynamic range, perceived loudness, timbre, onset rate, and general entropy.")

st.sidebar.subheader("Instrumentalness")
st.sidebar.write('Predicts whether a track contains no vocals. “Ooh” and “aah” sounds are treated as instrumental in this context. Rap or spoken word tracks are clearly “vocal”. The closer the instrumentalness value is to 1.0, the greater likelihood the track contains no vocal content. Values above 0.5 are intended to represent instrumental tracks, but confidence is higher as the value approaches 1.0.')

st.sidebar.subheader("Liveness")
st.sidebar.write("Detects the presence of an audience in the recording. Higher liveness values represent an increased probability that the track was performed live. A value above 0.8 provides strong likelihood that the track is live.")

st.sidebar.subheader("Loudness")
st.sidebar.write("The overall loudness of a track in decibels (dB). Loudness values are averaged across the entire track and are useful for comparing relative loudness of tracks. Loudness is the quality of a sound that is the primary psychological correlate of physical strength (amplitude). Values typical range between -60 and 0 db.")

st.sidebar.subheader("Speechiness")
st.sidebar.write("Speechiness detects the presence of spoken words in a track. The more exclusively speech-like the recording (e.g. talk show, audio book, poetry), the closer to 1.0 the attribute value. Values above 0.66 describe tracks that are probably made entirely of spoken words. Values between 0.33 and 0.66 describe tracks that may contain both music and speech, either in sections or layered, including such cases as rap music. Values below 0.33 most likely represent music and other non-speech-like tracks.")

st.sidebar.subheader("Valence")
st.sidebar.write("A measure from 0.0 to 1.0 describing the musical positiveness conveyed by a track. Tracks with high valence sound more positive (e.g. happy, cheerful, euphoric), while tracks with low valence sound more negative (e.g. sad, depressed, angry). ")
  
st.sidebar.subheader("Tempo")
st.sidebar.write("The overall estimated tempo of a track in beats per minute (BPM). In musical terminology, tempo is the speed or pace of a given piece and derives directly from the average beat duration.")
